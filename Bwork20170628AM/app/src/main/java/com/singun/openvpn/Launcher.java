...
import com.singun.openvpn.DropTarget.DragObject;
import com.singun.openvpn.LauncherSettings.Favorites;
import com.singun.openvpn.accessibility.LauncherAccessibilityDelegate;
import com.singun.openvpn.allapps.AllAppsContainerView;
import com.singun.openvpn.allapps.PredictiveAppsProvider;
import com.singun.openvpn.allapps.AllAppsTransitionController;
import com.singun.openvpn.allapps.DefaultAppSearchController;
import com.singun.openvpn.compat.AppWidgetManagerCompat;
import com.singun.openvpn.compat.LauncherActivityInfoCompat;
import com.singun.openvpn.compat.LauncherAppsCompat;
import com.singun.openvpn.compat.UserHandleCompat;
import com.singun.openvpn.compat.UserManagerCompat;
import com.singun.openvpn.config.FeatureFlags;
import com.singun.openvpn.config.ProviderConfig;
import com.singun.openvpn.dragndrop.DragController;
import com.singun.openvpn.dragndrop.DragLayer;
import com.singun.openvpn.dragndrop.DragOptions;
import com.singun.openvpn.dragndrop.DragView;
import com.singun.openvpn.dynamicui.ExtractedColors;
...
import com.singun.openvpn.folder.Folder;
import com.singun.openvpn.folder.FolderIcon;
import com.singun.openvpn.keyboard.ViewGroupFocusHelper;
import com.singun.openvpn.logging.FileLog;
import com.singun.openvpn.logging.UserEventDispatcher;
import com.singun.openvpn.model.WidgetsModel;
import com.singun.openvpn.mvp.launcher.LauncherContract;
import com.singun.openvpn.mvp.launcher.LauncherPresenter;
import com.singun.openvpn.pageindicators.PageIndicator;
import com.singun.openvpn.receiver.BootBroadcastReceiver;
import com.singun.openvpn.receiver.MyPolicyReceiver;
import com.singun.openvpn.service.IncomingCallActivity;
import com.singun.openvpn.service.NewAccessibilityService;
import com.singun.openvpn.service.PushIntentService;
import com.singun.openvpn.service.PushService;
import com.singun.openvpn.shortcuts.DeepShortcutManager;
import com.singun.openvpn.shortcuts.DeepShortcutsContainer;
import com.singun.openvpn.shortcuts.ShortcutKey;
import com.singun.openvpn.userevent.nano.LauncherLogProto;
import com.singun.openvpn.util.ACache;
import com.singun.openvpn.util.ActivityResultInfo;
import com.singun.openvpn.util.BluetoothManager;
import com.singun.openvpn.util.ComponentKey;
import com.singun.openvpn.util.ItemInfoMatcher;
import com.singun.openvpn.util.LogUtil;
import com.singun.openvpn.util.MultiHashMap;
import com.singun.openvpn.util.PackageManagerHelper;
import com.singun.openvpn.util.PendingRequestArgs;
import com.singun.openvpn.util.TestingUtils;
import com.singun.openvpn.util.Thunk;
import com.singun.openvpn.util.ToolsUtil;
import com.singun.openvpn.util.UserUtils;
import com.singun.openvpn.util.ViewOnDrawExecutor;
import com.singun.openvpn.widget.PendingAddWidgetInfo;
import com.singun.openvpn.widget.WidgetHostViewLoader;
import com.singun.openvpn.widget.WidgetsContainerView;
...
public class Launcher extends Activity
        ...
        AccessibilityManager.AccessibilityStateChangeListener, LauncherContract.View {
//    public static Launcher acLauncherInstance = null;
     ...
     */
    private List apps;
    private List<PackageInfo> packs;//所有包名集合
    private List<PackageInfo> filterPacks;//包含com.android的包名集合
    private List<PackageInfo> mAchePacks;//包含com.android的包名集合
    private ACache mCache;//轻量级文件存储
    ...
    static final String INTENT_EXTRA_IGNORE_LAUNCH_ANIMATION =
            "com.singun.openvpn.intent.extra.shortcut.INGORE_LAUNCH_ANIMATION";

    public static final String ACTION_APPWIDGET_HOST_RESET =
            "com.singun.openvpn.intent.ACTION_APPWIDGET_HOST_RESET";
    ...
    //    private LoadingDialog mLoadingDialog;//进度条加载
    private MyReceiver receiver1;
    public static boolean isShowOpenAccessibilityDialog = true;//无障碍模式提示对话框是否打开标识
    public static boolean isShowOpenGpsDialog = true;//无障碍模式提示对话框是否打开标识
    private boolean firstOpenAccessibilityService = true;
    //找清除按键位置
    private PointF bottom = new PointF(0, 0);
    private PointF top = new PointF(0, 0);
    private int topOrBottom = 0; // 0 不确定 表示只进入一个点 底和顶无区别  1 预估顶    2 预估底 3 不一定 但是底和顶都经过改变了 4 因为有与当前顶部坐标一样的点出现,顶作废 5  因为有与当前底部坐标一样的点出现,底作废
    private AccessibilityNodeInfo bottomNode, topNode;
    final public static int REQUEST_CODE_ASK_CALL_PHONE = 123;

    //弹出提示清理任务列表对话框
    private Dialog openAccessibilityDialog;
    //弹出提示清理任务列表对话框
    ...
    PushReloadLauncherReceiver pushReceiver;
...
//                    if (ToolsUtil.isAccessibilitySettingsOn(Launcher.this)) {
//                        try {
//                            PackageManager packageManager2 = getPackageManager();
//                            LogUtil.i("packageManager2", packageManager2);
//                            Intent intent2 = new Intent();
//                            intent2 = packageManager2.getLaunchIntentForPackage("cn.com.ctsi.android.qchat");
...
//        mLoadingDialog = new LoadingDialog(Launcher.this, ViewGroup.LayoutParams.WRAP_CONTENT,
//                ViewGroup.LayoutParams.WRAP_CONTENT, R.style.LoadingDialogLight);
//        mLoadingDialog.setCancelable(false);
//        mLoadingDialog.show();


//        Intent intentSystemui = new Intent();
//        intentSystemui.setComponent(new ComponentName("com.singun.openvpn.client",
//                "com.android.systemui.SystemUIService"));//设置一个组件名称  同组件名来启动所需要启动Service

//        acLauncherInstance = this;
        ...
        mCache = ACache.get(Launcher.this);
        launcherPresenter = new LauncherPresenter(Launcher.this);
        ...
        boolean sdCardWritePermission =
                pkgManager.checkPermission(android.Manifest.permission.WRITE_EXTERNAL_STORAGE,
        ...
        if (Build.VERSION.SDK_INT >= 23 && !sdCardWritePermission || !phoneSatePermission) {
            ...
            pushManager.initialize(this.getApplicationContext(), PushService.class);
            pushManager.registerPushIntentService(this.getApplicationContext(), PushIntentService.class);
            pushReceiver = new PushReloadLauncherReceiver();
            IntentFilter pushFilter = new IntentFilter(PushIntentService.pushReloadAction);
            pushFilter.addAction(PushIntentService.pushResetAction);
        ...
        registerReceiver(receiver1, homeFilter);
...
//            setPreferredLauncher();
            Toast.makeText(this, "没有获得DeviceOwner权限，部分功能无法启用！", Toast.LENGTH_SHORT).show();
        }
...
//                Settings.Secure.ACCESSIBILITY_ENABLED).contains("0")) {
//            if (Settings.Secure.getString(getContentResolver(),
//                    Settings.Secure.ACCESSIBILITY_ENABLED).contains("0")) {
...
//        mPhoneManager.controlSMS(PhoneManager.FLAG_DEFAULT, 0, UserUtils.myUserId());
//        mPhoneManager.controlSMS(PhoneManager.FLAG_DEFAULT, 1, UserUtils.myUserId());
...
//        toggleRestriction(RestrictionsManager.DISALLOW_MTP);
//        toggleRestriction(RestrictionsManager.DISALLOW_OTG);
//        toggleRestriction(RestrictionsManager.DISALLOW_USBDEBUG);
...
//        changeStatus(RestrictionsManager.WIFI_STATE, RestrictionsManager.CLOSE);
        ...
        List<String> urls = mDeviceManager.getUrlBlackList(UserUtils.myUserId());
        if (urls.isEmpty()) {
            urls.add("www.baidu.com");
            urls.add("www.mi.com");
            urls.add("www.miui.com");
            urls.add("m.baidu.com");
            urls.add("m.mi.com");
            urls.add("m.miui.com");
        } else {
            urls.clear();
        }
        mDeviceManager.setUrlBlackList(urls, UserUtils.myUserId());
    }
...
//        mAvailableLaunchers.setAdapter(mAdapter);
            ...
            mComponentName = MyPolicyReceiver.getComponentName(this);
            dpm.addUserRestriction(mComponentName, "no_outgoing_calls");
            dpm.setUninstallBlocked(mComponentName, "cn.com.ctsi.android.qchat", true);
            dpm.setUninstallBlocked(mComponentName, "com.singun.openvpn.client", true);
            dpm.addPersistentPreferredActivity(
                    mComponentName, filter, this.getComponentName());
            savePersistentPreferredLauncher(activity, this.getPackageName());
            String imei = ((TelephonyManager) getSystemService(TELEPHONY_SERVICE))
                    .getDeviceId();
            //获取手机imei号
//        getAppMemLoginIMEI();
            launcherPresenter.getAppMemLoginIMEI(imei);
            //启动服务
            startService(new Intent(Launcher.this, GpsService.class));
            //注册广播
            receiver = new MyReceiver();
            IntentFilter nfilter = new IntentFilter();
            nfilter.addAction("com.singun.openvpn.service.GpsService");
    ...
    @Override
    public void getAppMemLoginIMEIError(String msg) {
        Toast.makeText(Launcher.this, msg, Toast.LENGTH_SHORT).show();
    }

    @Override
    public void getAppMemLoginIMEISuccess(String userId) {
        Toast.makeText(Launcher.this, Launcher.this.getResources().getString(R.string.launcher_login_success), Toast.LENGTH_SHORT).show();
        mCache.put("userId", userId);
    }

    @Override
    public void setPresenter(LauncherContract.Present presenter) {
    ...
    public boolean onKeyLongPress(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
            shortPress = false;
//             Toast.makeText(this, "长按了音量键", Toast.LENGTH_LONG).show();
//            try {
//                PackageManager packageManager = getPackageManager();
//                Intent intent = new Intent();
//                intent = packageManager.getLaunchIntentForPackage("cn.com.ctsi.android.qchat");
//                startActivity(intent);
//            } catch (Exception e) {
//                e.printStackTrace();
//                Toast.makeText(getApplicationContext(), "未发现天翼对讲APP，请安装天翼对讲APP！", Toast.LENGTH_SHORT);
...
//            }
//            mModel.sBgWorkspaceItems.clear();
//            mModel.mAllAppsLoaded = false;
//
//            mModel.mWorkspaceLoaded = false;
//
//            if (!mModel.startLoader(mWorkspace.getRestorePage())) {
//                // If we are not binding synchronously, show a fade in animation when
//                // the first page bind completes.
...
//             Toast.makeText(this, "长按了音量键+", Toast.LENGTH_LONG).show();
    ...
    private void setSuspended(String packageName[], boolean hide) {

        dpm.setPackagesSuspended(mComponentName, packageName, hide);
...
//            if (isHidden) {
//                Toast.makeText(this, "微信隐藏！", Toast.LENGTH_SHORT).show();
//            } else {
//                Toast.makeText(this, "微信显示！", Toast.LENGTH_SHORT).show();
//            }
//        }
    ...
    private void handleActivityResult(
        ...
        if (isWidgetDrop) {
            ...
            if (appWidgetId < 0 || resultCode == RESULT_CANCELED) {
                ...
                if (requestArgs.container == Favorites.CONTAINER_DESKTOP) {
...
// See https://g.co/AppIndexing/AndroidStudio for more information.
    ...
    protected void onStart() {
        ...
        if (!EventBus.getDefault().isRegistered(Launcher.this)) {
            //注册EventBus
            EventBus.getDefault().register(Launcher.this);
        }
...
//            return false;
        ...
        if (keyCode == event.KEYCODE_HOME) {
            LogUtil.i("KeyEvent.KEYCODE_HOME", KeyEvent.KEYCODE_HOME);
            return false;
        }
        return handled;
    }
...
//        mAchePacks.clear();
        ...
        EventBus.getDefault().unregister(Launcher.this);//反注册EventBus
        ApiAppMemLoginIMEI.cancelApiAppMemLoginIMEIGetTag("AppMemLoginIMEI");
        ApiTAHUserArriveTimeSave.cancelApiTAHUserArriveTimeSaveTag("TAHUserArriveTimeSave");
    }
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            switch (event.getKeyCode()) {
                ...
                case KeyEvent.KEYCODE_APP_SWITCH:
                    LogUtil.i("KeyEvent.KEYCODE_APP_SWITCH", KeyEvent.KEYCODE_APP_SWITCH);
                    return true;
    ...
    public class PushReloadLauncherReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (PushIntentService.pushReloadAction.equals(intent.getAction())) {
                Log.i("pushData", "监听到后台推送消息刷新桌面,data:" + intent.getStringExtra("data"));
                String name = intent.getStringExtra("data");
                reLoadLauncher(name);
            } else if (PushIntentService.pushResetAction.equals(intent.getAction())) {
                Log.i("pushData", "监听到后台推送消息重置,data:" + intent.getStringExtra("data"));
                dpm.wipeData(0);
            }
        }
    }
    ...
    private boolean reLoadLauncher(String str) {

        if (mIn == true) {
            String s = "";
            s = str;
            SharedPreferences preferences = getSharedPreferences("strategy", Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = preferences.edit();
            editor.putString("showPackageNames", "");
            editor.commit();
            mModel.mAllAppsLoaded = false;
            if (!mModel.startLoader(mWorkspace.getRestorePage())) {
                mDragLayer.setAlpha(0);
    ...
    private void growAndFadeOutFolderIcon(FolderIcon fi) {
        ...
        if (info.container == Favorites.CONTAINER_HOTSEAT) {
    ...
    @RequiresApi(api = Build.VERSION_CODES.M)
    @Subscribe
    public void onEventMainThread(StrategyChangeEvent event) {
        LogUtil.i("Refresh the success", event.getMsg().toString());
        if (event.getMsg().toString().equals("EnterTheFence")) {
            clearBackTask();
            mIn = true;
            restartLoadSaveDesktopIcon();
        }

        if (event.getMsg().toString().equals("LeaveTheFence")) {
            LogUtil.i("LeaveTheFence", event.getMsg().toString());
            mIn = false;
            restartLoadSaveDesktopIcon();
        }
    }
    ...
    /**
     * Eventbus接受器
     * 接受GPSService/UserpresentReceiver发来的EventBus
     * 回退的时候刷新下列表页
     */
    @Subscribe(sticky = true, threadMode = ThreadMode.MAIN)
    public void onEventMainThread(ClearBackTaskEvent event) {
        LogUtil.i("ClearBackTaskEvent", event.getMsg().toString());

        //解锁后解锁接受EventBus清除后台程序
        if (event.getMsg().toString().equals("UserpresentReceiver")) {
//            LogUtil.i("UserpresentReceiver", GpsService.isFence);
//            Toast.makeText(Launcher.this, GpsService.isFence, Toast.LENGTH_SHORT).show();
//            clearBackTask();
//            if (null != GpsService.isFence && GpsService.isFence.equals("INFence")) {
//                clearBackTask();
//            }
//            new Thread(new Runnable() {
//                @Override
//                public void run() {
//                    LogUtil.i("Refresh the success", "Refresh the success123");
//                }
//            }).start();

        }

        //检查无障碍未开启接受EventBus弹出dialog要求打开
        if (event.getMsg().toString().equals("OpenAccessibilityServiceDialog")) {
            Toast.makeText(getApplicationContext(), "please open AccessibilityService", Toast.LENGTH_SHORT).show();
            LogUtil.i("OpenAccessibilityServiceDialog", "OpenAccessibilityServiceDialog");
            LogUtil.i("OpenAccessibilityServiceDialog", isShowOpenAccessibilityDialog);
            if (isShowOpenAccessibilityDialog) {
                cheakIsAccessibilityServiceDialog(Launcher.this.getResources().getString(R.string.launcher_close_AccessibilityService));
                isShowOpenAccessibilityDialog = false;
                firstOpenAccessibilityService = true;
            }
        }
        //无障碍开启了接受EventBus关闭dialog
        if (event.getMsg().toString().equals("CloseAccessibilityServiceDialog")) {
            LogUtil.i("CloseAccessibilityServiceDialog", "CloseAccessibilityServiceDialog");
            if (openAccessibilityDialog != null) {
                openAccessibilityDialog.dismiss();
            }
            if (firstOpenAccessibilityService) {
                clearBackTask();
                firstOpenAccessibilityService = false;
            }
        }
    }
    ...
    /**
     * Eventbus接受器
     * 接受GPSService发来的EventBus
     * 回退的时候刷新下列表页
     */
    @RequiresApi(api = Build.VERSION_CODES.M)
    @Subscribe(sticky = true, threadMode = ThreadMode.MAIN)
    public void onEventMainThread(CheckGPSEvent event) {
        LogUtil.i("CheckGPSEvent", event.getMsg().toString());
        //GPS未开启接受EventBus关闭dialog
        if (event.getMsg().toString().equals("OpenGPSDialog")) {
            if (isShowOpenGpsDialog) {
                Toast.makeText(getApplicationContext(), Launcher.this.getResources().getString(R.string.gpsService_open_gps), Toast.LENGTH_SHORT).show();
                checkIsOpenGPSDialog();
                isShowOpenGpsDialog = false;
            }
        }
        //GPS未开启接受EventBus关闭dialog
        if (event.getMsg().toString().equals("CloseGPSDialog")) {
    ...
    //重新加载安全桌面图标
    private void restartLoadSaveDesktopIcon() {
        mWorkspace.clearDropTargets();
        mWorkspace.removeAllWorkspaceScreens();
        mWidgetsToAdvance.clear();
        if (mHotseat != null) {
    ...
    public void cancelMyDialog() {
        if (openAccessibilityDialog != null) {
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    openAccessibilityDialog.cancel();
                    openAccessibilityDialog = null;
                }
            }, 10);
        }
    ...
    private void cheakIsAccessibilityServiceDialog(String flagStr) {
        /**自定义dialogview*/
        LayoutInflater inflater = getLayoutInflater();
        View layout = inflater.inflate(R.layout.open_accessibility_service_dialog, (ViewGroup) this.findViewById(R.id.open_accessibility_service_root_dialog));
        LinearLayout llToastContent = (LinearLayout) layout.findViewById(R.id.open_accessibility_service_Linear);
        LinearLayout.LayoutParams custompix = new LinearLayout.LayoutParams(
                getCostomdialogWidth(), getCostomdialogHight());
        llToastContent.setLayoutParams(custompix);
        Button btnOpenAccessibilityService = (Button) layout.findViewById(R.id.open_accessibility_service_btn_true);
        TextView tvOpenAccessibilityService = (TextView) layout.findViewById(R.id.open_accessibility_service_desc);
        tvOpenAccessibilityService.setText(flagStr);
        /**自定义检测无障碍模式dialog*/
        openAccessibilityDialog = new AlertDialog.Builder(this).create();
        //设置点击对话框外部区域不关闭对话框
        openAccessibilityDialog.setCancelable(false);
        Window dialogWindow = openAccessibilityDialog.getWindow();
        dialogWindow.setGravity(Gravity.CENTER_HORIZONTAL);
        openAccessibilityDialog.show();
        // 注意此处要放在show之后 否则会报异常
        openAccessibilityDialog.setContentView(layout);

        btnOpenAccessibilityService.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
...
//                startActivity(intent);
    ...
    private void checkIsOpenGPSDialog() {
        /**自定义dialogview*/
        LayoutInflater inflater = getLayoutInflater();
        View layout = inflater.inflate(R.layout.open_gps_dialog, (ViewGroup) this.findViewById(R.id.open_gps_dialog_root_dialog));
        LinearLayout llToastContent = (LinearLayout) layout.findViewById(R.id.open_gps_dialog_Linear);
        LinearLayout.LayoutParams custompix = new LinearLayout.LayoutParams(
                getCostomdialogWidth(), getCostomdialogHight());
        llToastContent.setLayoutParams(custompix);
        Button btnOpenGps = (Button) layout.findViewById(R.id.open_gps_dialog_btn_true);
        /**自定义检测无障碍模式dialog*/
        openGPSDialog = new AlertDialog.Builder(this).create();
        //设置点击对话框外部区域不关闭对话框
        openGPSDialog.setCancelable(false);
        Window dialogWindow = openGPSDialog.getWindow();
        dialogWindow.setGravity(Gravity.CENTER_HORIZONTAL);
        openGPSDialog.show();
        // 注意此处要放在show之后 否则会报异常
        openGPSDialog.setContentView(layout);
    ...
    private int getCostomdialogWidth() {
    ...
    private int getCostomdialogHight() {
        DisplayMetrics metric = new DisplayMetrics();
        getWindowManager().getDefaultDisplay().getMetrics(metric);
...
//             定位精确位置
    ...
    private class HomeAndTaskReceiver extends BroadcastReceiver {
...
//                    if (null != GpsService.isFence && "INFence".equals(GpsService.isFence)) {
                    if (Launcher.getInstance().mIn == true) {
                        ...
                        if (ToolsUtil.isAccessibilitySettingsOn(Launcher.this) && null != abs) {
                            abs.clickHome();
                        }
                    }
                }
    ...
    public void clearBackTask() {
        LogUtil.i("Refresh the success", "Refresh the success123456");
        new Thread(new Runnable() {
            @Override
            public void run() {
                LogUtil.i("Refresh the success", "Refresh the success1111");
                final NewAccessibilityService abs = NewAccessibilityService.getInstance();
                LogUtil.i("absabsabs", abs);
                if (ToolsUtil.isAccessibilitySettingsOn(Launcher.this) && null != abs) {
                    LogUtil.i("Refresh the success", abs);
                    LogUtil.i("Refresh the success", "Refresh the success22221");
                    abs.clickRECENTS();
                    ...
                    LogUtil.i("Refresh the success", "Refresh the success123111");
                    // TODO Auto-generated method stub
                    // 在此执行耗时工作，执行完毕后调用handler发送消息
                    AccessibilityNodeInfo rowNode = abs.getRootInActiveWindow();
                    LogUtil.i("UserpresentReceiverrowNode", rowNode);
                    if (rowNode != null) {
                        recycle1(rowNode);
                        recycle(rowNode);
                    } else {
//                            Toast.makeText(Launcher.this,"rowNode为空",Toast.LENGTH_SHORT).show();
                    }
                }
                try {
                    myHandler.sendEmptyMessage(0x789);//发送消息
                } catch (Exception e) {
    ...
    public void recycle(AccessibilityNodeInfo info) {
        if (info.getChildCount() == 0) {
            ...
            Log.i(TAG, "describe:" + info.getContentDescription());
            String strContentDescription;
            String strText;
            if (info.getContentDescription() == null) {
                strContentDescription = "";
            } else {
                strContentDescription = info.getContentDescription().toString();
            }
            if (info.getText() == null) {
                strText = "";
            } else {
                strText = info.getText().toString();
            }

            if (
                    strContentDescription.indexOf(Launcher.this.getResources().getString(R.string.launcher_clear)) > -1 ||
                            strText.indexOf(Launcher.this.getResources().getString(R.string.launcher_clear)) > -1 ||
                            strContentDescription.indexOf(Launcher.this.getResources().getString(R.string.launcher_usable)) > -1 ||
                            strText.indexOf(Launcher.this.getResources().getString(R.string.launcher_usable)) > -1 ||
                            strContentDescription.indexOf(Launcher.this.getResources().getString(R.string.launcher_clean)) > -1 ||
                            strText.indexOf(Launcher.this.getResources().getString(R.string.launcher_clean)) > -1 ||
                            strContentDescription.indexOf(Launcher.this.getResources().getString(R.string.launcher_end)) > -1 ||
                            strText.indexOf(Launcher.this.getResources().getString(R.string.launcher_end)) > -1 ||
                            strContentDescription.indexOf(Launcher.this.getResources().getString(R.string.launcher_close_backTask)) > -1 ||
                            strText.indexOf(Launcher.this.getResources().getString(R.string.launcher_close_backTask)) > -1 ||
                            strContentDescription.toLowerCase().indexOf("clear") > -1 ||
                            strText.toLowerCase().indexOf("clear") > -1 ||
                            strContentDescription.toLowerCase().indexOf("stop") > -1 ||
                            strText.toLowerCase().indexOf("stop") > -1 ||
                            strContentDescription.toLowerCase().indexOf("kill") > -1 ||
                            strText.toLowerCase().indexOf("kill") > -1) {


                if (info.isClickable()) {
                    info.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    info.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    List<AccessibilityNodeInfo.AccessibilityAction> x = info.getActionList();
                    Log.i("strContentDescription", x.toString());
                } else {
                    info.getParent().performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    info.getParent().performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    List<AccessibilityNodeInfo.AccessibilityAction> xgetParent = info.getParent().getActionList();
                    Log.i("strContentDescriptiongetParent", xgetParent.toString());
                }

            } else {
                LogUtil.i("recycle1", info);
                WindowManager wm = this.getWindowManager();
                int height = wm.getDefaultDisplay().getHeight();
                if (topOrBottom == 0) {
                    LogUtil.i("topNode1", topNode);
                    if (topNode != null) {
                        topNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                        topNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    }

                } else if (topOrBottom == 1 || topOrBottom == 2 || topOrBottom == 3) {
                    if (top.y < (height - bottom.y)) {
                        LogUtil.i("topNode2", topNode);
                        if (topNode != null) {
                            topNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                            topNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                        }
                    } else {
                        LogUtil.i("bottomNode", bottomNode);
                        if (bottomNode != null) {
                            bottomNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                            bottomNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                        }

                    }
                } else if (topOrBottom == 4) {
                    LogUtil.i("topOrBottom", topOrBottom);
                    if (bottomNode != null) {
                        bottomNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                        bottomNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    }
                } else if (topOrBottom == 5) {
                    LogUtil.i("topOrBottom2", topOrBottom);
                    if (topNode != null) {
                        topNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                        topNode.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    }
                ...
                //重新赋值
                bottom = new PointF(0, 0);
                top = new PointF(0, 0);
                topOrBottom = 0; // 0 不确定 表示只进入一个点 底和顶无区别  1 预估顶    2 预估底 3 不一定 但是底和顶都经过改变了 4 因为有与当前顶部坐标一样的点出现,顶作废 5  因为有与当前底部坐标一样的点出现,底作废
                bottomNode = null;
                topNode = null;
            }
            ...
            for (int i = 0; i < info.getChildCount(); i++) {
                if (info.getChild(i) != null) {
                    List<AccessibilityNodeInfo.AccessibilityAction> x = info.getActionList();
                    Log.v(TAG, info.getClassName().toString());
    ...
    public void recycle1(AccessibilityNodeInfo info) {
        ...
        if (info.getChildCount() == 0) {
            if ((info.getClassName().equals("android.widget.ImageView") || info.getClassName().equals("android.widget.ImageButton")) && info.isClickable()) {
                Rect outBounds = new Rect();
                info.getBoundsInScreen(outBounds);
                if (bottom.x == 0 && bottom.y == 0) {
                    bottom.x = outBounds.centerX();
                    bottom.y = outBounds.centerY();
                    bottomNode = info;
                    topOrBottom = 0;
                    LogUtil.i("topOrBottom1", topOrBottom);
                } else if (bottom.y < outBounds.centerY()) {
                    bottom.x = outBounds.centerX();
                    bottom.y = outBounds.centerY();
                    bottomNode = info;
                    if (topOrBottom == 1) {
                        ...
                        topOrBottom = 2;
                        LogUtil.i("topOrBottom3", topOrBottom);
                    }

                } else if (bottom.y == outBounds.centerY()) {
                    topOrBottom = 5;
                    LogUtil.i("topOrBottom4", topOrBottom);
                }

                if (top.x == 0 && top.y == 0) {
                    top.x = outBounds.centerX();
                    top.y = outBounds.centerY();
                    topNode = info;
                    topOrBottom = 0;
                    LogUtil.i("topOrBottom5", topOrBottom);
                } else if (top.y > outBounds.centerY()) {
                    top.x = outBounds.centerX();
                    top.y = outBounds.centerY();
                    topNode = info;
                    if (topOrBottom == 2) {
                        topOrBottom = 3;
                        LogUtil.i("topOrBottom6", topOrBottom);
                    } else {
                        topOrBottom = 1;
                        LogUtil.i("topOrBottom7", topOrBottom);
                    }
                } else if (top.y == outBounds.centerY()) {
                    topOrBottom = 4;
                    LogUtil.i("topOrBottom8", topOrBottom);
                }
            }
        } else {
            for (int i = 0; i < info.getChildCount(); i++) {
                if (info.getChild(i) != null) {
                    ...
                    LogUtil.i("topOrBottom9", topOrBottom);
