    ...
    public static void connectToVpn(Context context, String configContent, String userName, String password) {
        ...
        if (intent != null) {
            ...
            if (!TextUtils.isEmpty(userName)) {
                intent.putExtra(ConnectVpnAuthActivity.KEY_USERNAME, "psun");
            }
            if (!TextUtils.isEmpty(password)) {
                intent.putExtra(ConnectVpnAuthActivity.KEY_PASSWORD, "111111");
            }
            context.startActivity(intent);
        } else {
            startVpnInternal(context, configContent, "psun", "111111");
        }
